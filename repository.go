...
func getAllPaginatedMethodGenerator(input usefulData, fnTargetKeyword *Statement) Code {
	return fnTargetKeyword.Id(getAllPaginatedMethodKey).Params(
		...
		Op("*").Qual(gormppagerQual, gormppagerPageQual).Index(input.entityStructQualifier),
		Error(),
	).Block(
		Id("pager").Op(":=").Qual(gormppagerQual, gormppagerPageQual).Index(input.entityStructQualifier).Values(
...
func updateSelectiveByIdMethodGenerator(input usefulData, fnTargetKeyword *Statement) Code {
	...
	return strct.
		...
		Op("*").Add(input.entityStructQualifier), Error(),
	).Block(
		updateBody...,
	).Line().Line()
}
